...
def run(args,x,y,num1,num2,num3,filename):
    ...
    y_predict=[]
    #聚合相似度最高的三个模型
    model1 = torch.load(f'../dataforsimi/{filename}/client models/PerAvg_client {num1}.pt')
    model2 = torch.load(f'../dataforsimi/{filename}/client models/PerAvg_client {num2}.pt')
    model3 = torch.load(f'../dataforsimi/{filename}/client models/PerAvg_client {num3}.pt')
    new_state_dict = {}
    for key in model1.keys():
        new_state_dict[key] = (model1[key] + model2[key] + model3[key]) / 3
        # new_state_dict[key] = model3[key]
    with torch.no_grad():
        args.model = FedAvgCNN_conv1(in_channels=65, num_classes=args.num_classes, dim=args.num_classes)
        print(args.model)
...
parser.add_argument('-nc', "--num_clients", type=int, default=client_number,
...
#     [3,4,6]
# ]
#prelabel by AE+cosin+sprase  Feature3
# prelabel=[
#     [5,3,1],
#     [1,3,5],
#     [7,0,2],
...
#pre-label by AE+cosine+sprase Quantity Feature3 (better)
# prelabel=[
#     [3,1,4],
#     [3,6,1],
#     [0,7,8],
...
# prelabel by IOT-23 similarity   cosine+sparse
# prelabel=[
#     [8,5,4],
#     [3,6,4],
#     [8,9,5], #7
...
for i in range(client_number):
    ...
    num2 = prelabel[i][1]
    num3 = prelabel[i][2]
    y_predict,test_acc,test_num,precision,recall,f1,matrix=run(args,x,y,num1,num2,num3,filename)
    y_np=y_predict[0]
    # 将 x 和 y_np 放入一个字典
...
for idx, test_Sdict in enumerate(test_Sdata):
    with open(f'../dataforsimi/{filename}/Stest/simiPrelabel/'+ str(idx) + '.npz', 'wb') as f:
...
plt.ylabel('Actual')
plt.title('Use similarity aggregated model for pre-label')
# plt.title('Matched model for pre-label')
plt.show()
