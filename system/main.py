...
def run(args):
    ...
    for i in range(args.prev, args.times):
        ...
        if model_str == "mlr": # convex
            ...
            elif "CICIDS2017" in args.dataset:
            ...
            elif "IOT-23" in args.dataset:
                # Conv1d的通道数是指词向量的维度，Conv2d的通道数是指颜色通道
                # 比如：黑白图的通道数是1和RGB彩色图的通道数为3或者设置更多的颜色通道数
                # 几个类别几个dim
                args.model = FedAvgCNN_conv1(in_channels=25, num_classes=args.num_classes, dim=args.num_classes)
            elif "TON-IoT" in args.dataset:
                args.model = FedAvgCNN_conv1(in_channels=31, num_classes=args.num_classes, dim=args.num_classes)
            else:
                args.model = FedAvgCNN(in_features=65, num_classes=args.num_classes, dim=10816).to(args.device)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-data', "--dataset", type=str, default="IOT-23")
    ...
    parser.add_argument('-jr', "--join_ratio", type=float, default=0.6,
