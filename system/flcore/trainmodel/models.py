...
#处理一维数据的CNN
class FedAvgCNN_conv1(nn.Module):
    def __init__(self, in_channels=1, num_classes=9, dim=1024):
        super().__init__()
        ...
        self.model2 = nn.Sequential(
            # nn.Linear(in_features=224, out_features=num_classes, bias=True), #CICIDS2017
            nn.Linear(in_features=64, out_features=num_classes, bias=True), # IOT-23
            # nn.Linear(in_features=96, out_features=num_classes, bias=True),  # TON-IoT
    ...
    def forward(self, x):
       # print("x initial",x.shape) #[40,65] 每组为40，特征为65 ||[40,25]
        x = x.reshape(-1, 1, 25)  # -1表示让系统自己计算有多少个样本，这个操作的目的就是把数据转换为3维的 [40,1,65]  65||25||31
       # print(x)
       # print("x after reshape:",x.shape)
       # x=x.permute(1,0) #维度转换 [65,40] 一批batch，每个有65个维度，有40个用户
       # print("size after permute", x.shape)
        out = self.model1(x) #[40,224]
        out=self.model2(out)
        out = torch.flatten(out, 1)
        out = self.fc1(out)
        out = self.fc(out)
